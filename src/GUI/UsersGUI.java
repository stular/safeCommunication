...
public class UsersGUI extends JFrame {
	...
	private static int videoHight, videoWidth;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					UsersGUI frame = new UsersGUI(userName);
	 ...
	 * Create the frame.
	 */
	public UsersGUI(String username) {
		userName = username;

		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		setBounds(20, 20, 459, 345);
		UsersGUI.this.setTitle("Camera Viewer - " + username);
		...
		videoSettingsBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				comboBox.setModel(new DefaultComboBoxModel(new String[] { "Please select resolution:", "1024x700 (4:3)",
						"800x600 (4:3)", "640x480 (4:3)", "480x360 (4:3)", "320x240 (4:3)", "240x180 (4:3)",
						"160x120 (4:3)", "1280x720 (16:9)", "800x450 (16:9)", "640x360 (16:9)", "480x270 (16:9)",
						"320x180 (16:9)", "160x90 (16:9)", "1280x800 (16:10)", "176x144" }));
				comboBox.setBounds(10, 21, 131, 20);
				JSpinner spinner = new JSpinner();
				spinner.setModel(new SpinnerNumberModel(5, 5, 30, 1));
				spinner.setBounds(151, 21, 83, 20);
				final JComponent[] inputs = new JComponent[] { new JLabel("Resolution"), comboBox,
						new JLabel("Frequency"), spinner, };
				int result = JOptionPane.showConfirmDialog(null, inputs, "Video Settings Dialog",
		...
		videoSettingsBtn.setBounds(10, 162, 179, 23);
		...
		cameraSettingsBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				camera1 = dbm.getCameraPrivilegeForSingleUser(username, 1);
				camera2 = dbm.getCameraPrivilegeForSingleUser(username, 2);
				camera3 = dbm.getCameraPrivilegeForSingleUser(username, 3);
				camera4 = dbm.getCameraPrivilegeForSingleUser(username, 4);
				...
				table.setModel(new DefaultTableModel(
						new Object[][] { { "Camera 1", "Bed Room", "192.168.20.246", "10236", camera1 },
						...
						new String[] { "Name", "Location", "IP Address", "Port no.", "View" }) {
					...
					public boolean isCellEditable(int row, int column) {
						if (dbm.getCameraPrivilege(username, row + 1) == false)
							return false;
						return true;
					}
				});
				...
				int result = JOptionPane.showConfirmDialog(null, table, "Cameras viewing settings Dialog",
						JOptionPane.OK_CANCEL_OPTION);
				if (result == JOptionPane.OK_OPTION) {
					if ((Boolean) table.getValueAt(0, 4) != camera1)
						if ((Boolean) table.getValueAt(0, 4))
							dbm.setCameraPrivilegeToPositiveForSingleUser(userName, 1);
						else
							dbm.setCameraPrivilegeToNegativeForSingleUser(userName, 1);
					if ((Boolean) table.getValueAt(1, 4) != camera2)
						if ((Boolean) table.getValueAt(1, 4))
							dbm.setCameraPrivilegeToPositiveForSingleUser(userName, 2);
						else
							dbm.setCameraPrivilegeToNegativeForSingleUser(userName, 2);
					if ((Boolean) table.getValueAt(2, 4) != camera3)
						if ((Boolean) table.getValueAt(2, 4))
							dbm.setCameraPrivilegeToPositiveForSingleUser(userName, 3);
						else
							dbm.setCameraPrivilegeToNegativeForSingleUser(userName, 3);
					if ((Boolean) table.getValueAt(3, 4) != camera4)
						if ((Boolean) table.getValueAt(3, 4))
							dbm.setCameraPrivilegeToPositiveForSingleUser(userName, 4);
						else
							dbm.setCameraPrivilegeToNegativeForSingleUser(userName, 4);

					for (int i = 0; i < table.getRowCount(); i++)
						if (dbm.getCameraPrivilege(userName, i + 1))
							if ((Boolean) table.getValueAt(i, 4) == true) {
								switch (i + 1) {
								case 1:
									cam1.setText("streaming");
									// imageStream("192.168.20.246", 5001, cam1.getName());
									camera1 = true;
									break;
								case 2:
		...
		cameraSettingsBtn.setBounds(10, 196, 179, 23);
		panel.add(cameraSettingsBtn);

		JToggleButton alarmTglBtn = new JToggleButton("Activate Motion Alarm");
		...
		alarmTglBtn.setBounds(10, 230, 179, 23);
		...
		logOutBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
		        ...
		        UsersGUI.this.dispose();
			}
		});
		logOutBtn.setBounds(10, 264, 179, 23);
		panel.add(logOutBtn);

		JPanel panel_1 = new JPanel();
		panel_1.setLayout(null);
		panel_1.setBorder(new TitledBorder(new LineBorder(new Color(0, 0, 0)), "Cameras", TitledBorder.LEADING,
				TitledBorder.TOP, null, new Color(0, 0, 0)));
		panel_1.setBounds(10, 0, 182, 154);
		panel.add(panel_1);

		cam4 = new JLabel("Cam4");
		...
		cam1.setBounds(6, 16, 80, 60);
		panel_1.add(cam1);

		cam2 = new JLabel("Cam2");
		...
		cam2.setBounds(6, 87, 80, 60);
		panel_1.add(cam2);

		cam3 = new JLabel("Cam3");
		...
		if (!connected) {
			...
			if (homeClient.isConnected()) {
				...
				if (dbm.getCameraPrivilegeForSingleUser(userName, 1)) {
					if (dbm.getCameraPrivilege(userName, 1))
					{
						cam1.setText("streaming");
						camera1 = false;
						// imageStream("192.168.20.246", 5001, cam1.getName());
				...
				if (dbm.getCameraPrivilegeForSingleUser(userName, 2)) {
					if (dbm.getCameraPrivilege(userName, 2))
					{
						cam2.setText("streaming");
						camera2 = false;
						// imageStream("192.168.20.247", 5001, cam2.getName());
				...
				if (dbm.getCameraPrivilegeForSingleUser(userName, 3)) {
					if (dbm.getCameraPrivilege(userName, 3))
					{
						cam3.setText("streaming");
						camera3 = false;
						// imageStream("192.168.20.248", 5001, cam3.getName());
				...
				if (dbm.getCameraPrivilegeForSingleUser(userName, 4)) {
					if (dbm.getCameraPrivilege(userName, 4))
					{
						cam4.setText("streaming");
						camera4 = false;
						// imageStream("192.168.20.249", 5001, cam4.getName());
	...
	public void setVideoResolution()
	{
		...
		case 1:
			...
			videoWidth = 1024;
			videoHight = 700;
			this.setBounds(20, 20, 262 + videoWidth, 65 + videoHight);
		...
		case 5:
			...
			videoWidth = 320;
			videoHight = 240;
			this.setBounds(20, 20, 262 + videoWidth, 381);
			this.contentPane.setBounds(0, 0, 435, 346);
			break;
		case 6:
			//Res = "240x180";
			videoWidth = 240;
		...
		case 10:
			...
			videoWidth = 640;
			videoHight = 360;
			this.setBounds(2, 20, 262 + videoWidth, 65 + videoHight);
			this.contentPane.setBounds(0 , 0, 240 + videoWidth, 30 + videoHight);
			break;
		case 11:
			//Res = "480x270";
			...
			videoHight = 270;
			this.setBounds(20, 20, 262 + videoWidth, 381);
			this.contentPane.setBounds(0, 0, 500, 346);
			break;
		case 12:
			//Res = "320x180";
			videoWidth = 320;
    ...
    private void imageStream(String IPAddress, int portNum, int index, JLabel viewLbl){
        ...
        this.homeClient.writeEncInt(320);
        this.homeClient.writeEncInt(240);
    ...
    class Streamer implements Runnable{
        ...
        private JLabel view;
        public Streamer(UsersGUI gui, JLabel viewLbl){
            this.gui=gui;
            this.view = viewLbl;
        }
