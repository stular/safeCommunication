...
public class UserManagementGUI extends JFrame {
	...
	public UserManagementGUI() {
		...
		UserManagementGUI.this.setTitle("Users Management Window");
		...
		addNewUserBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				while (flag) {
					password.setText("");
					int result = JOptionPane.showConfirmDialog(null, myPanel, "Add new user",
					...
					if (result == JOptionPane.OK_OPTION) {
						...
						else if(password.getText().length() < 8 && userName.getText().length() == 0)
							JOptionPane.showMessageDialog(null, "User name can NOT be empty and password must consist of 8 characters at least");							
						else if (userName.getText().length() == 0)
							JOptionPane.showMessageDialog(null, "User name can NOT be empty");
						else
							JOptionPane.showMessageDialog(null, "Password must consist of 8 characters at least");							
					}
					else
						flag = false;
				}
				...
				if (usersTable.getSelectedRow() == -1)
					...
					while (flag) {
						...
						result = JOptionPane.showConfirmDialog(null, myPanel,
								"Update " + userNameToUpdate + "\'s password", JOptionPane.CANCEL_OPTION);
						if (result == JOptionPane.OK_OPTION)
		...
		exitBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				UserManagementGUI.this.dispose();
			}
		});
		...
		tablePanel.setBorder(new TitledBorder(
				new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Users", TitledBorder.LEADING,
						TitledBorder.TOP, null, new Color(0, 0, 0)),
				"Users", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		usersTable.setModel(
				new DefaultTableModel(new Object[userNames.length][2], new String[] { "User Name", "Password" }) {
